...
class ParticleManager:
    ...
    def spawn_dream_particle(self):
        self.particles.append(Particle(
            ...
            max_life=8,
            color=random.choice([settings.DREAMY_PINK, settings.LIGHT_PURPLE, settings.SOFT_PINK]),
    ...
    def _draw_particle(self, screen, particle):
        ...
        if particle.particle_type == "heart":
            self._draw_heart(screen, particle.x, particle.y, particle.size, particle.color, alpha_ratio)
            ...
            pygame.draw.circle(screen, particle.color, (int(particle.x), int(particle.y)), max(1, size))
    ...
    def _draw_star(self, screen, x, y, size, color):
        points = [
            (x, y - size), (x - size//3, y - size//3), (x - size, y), (x - size//3, y + size//3),
            (x, y + size), (x + size//3, y + size//3), (x + size, y), (x + size//3, y - size//3),
        ]
