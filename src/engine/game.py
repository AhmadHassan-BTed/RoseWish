...
from src.utils.particles import Particle, ParticleManager
...
class GameEngine:
    ...
    def update(self, dt: float):
        ...
        if self.state == GameState.TITLE:
            ...
            if random.random() < 0.1:
                self.particles.spawn_rose_petal()
            if random.random() < 0.05:
                self.particles.spawn_heart(random.randint(100, settings.WINDOW_WIDTH - 100), -20)

        elif self.state == GameState.INTRO:
            self.background.set_time("day")
            if random.random() < 0.2:
                self.particles.spawn_snow()
            self._handle_partner_movement(dt)
            if self.state_timer < 4:
                self.message = settings.INTRO_MESSAGE
            else:
                self.message = ""
            ...
            if random.random() < 0.15:
                self.particles.spawn_snow()
            if random.random() < 0.1:
                self.particles.spawn_sparkle(
                    self.partner.x + random.randint(-30, 30),
                    self.partner.y + random.randint(-30, 30),
                )
            ...
            if self.state_timer > 2:
                self.partner.sleeping = True
            if random.random() < 0.15:
                self.particles.spawn_dream_particle()
            if 4 <= self.state_timer < 4.1:
            ...
            if random.random() < 0.1:
                self.particles.spawn_sparkle(
                    random.randint(0, settings.WINDOW_WIDTH),
            ...
            if random.random() < 0.15:
                self.particles.spawn_sparkle(
                    self.player.x + random.randint(-40, 40), self.player.y + random.randint(-40, 40)
                )
            if self.state_timer > 3:
                self.partner.sleeping = False
            if self.state_timer > 8:
                self.state = GameState.KISS
                self.state_timer = 0

        elif self.state == GameState.KISS:
            if abs(self.player.x - self.partner.x) > 60:
                self.player.x += 30 * dt
            ...
            if random.random() < 0.3:
                self.particles.spawn_heart(
                    self.partner.x + random.randint(-50, 50),
                    self.partner.y - random.randint(20, 60),
                )
            if random.random() < 0.2:
                self.particles.spawn_rose_petal()
            ...
            if random.random() < 0.4:
                self.particles.spawn_heart(
                    ...
                    random.randint(-20, settings.WINDOW_HEIGHT // 2),
                )
            if random.random() < 0.3:
                self.particles.spawn_sparkle(
                    random.randint(0, settings.WINDOW_WIDTH),
            ...
            if self.state_timer < 5:
                self.message = settings.MARRIAGE_MESSAGE_1
            elif self.state_timer < 10:
                self.message = settings.MARRIAGE_MESSAGE_2
            else:
                self.message = ""
    ...
    def _handle_partner_movement(self, dt: float):
        ...
        self.partner.y = max(
            settings.WINDOW_HEIGHT - 250, min(settings.WINDOW_HEIGHT - 100, self.partner.y)
        )
    ...
    def render(self):
        ...
        if self.player.x > -100:
            self.player.draw(self.renderer.get_screen())
    ...
    def draw_message(self):
        ...
        curr = ""
        for w in words:
            if font.size(curr + w + " ")[0] < settings.WINDOW_WIDTH - 100:
                curr += w + " "
            else:
    ...
    def draw_title_screen(self):
        ...
        color = (
            int(settings.ROSE_RED[0] * (0.7 + 0.3 * pulse)),
            int(settings.ROSE_RED[1] * (0.7 + 0.3 * pulse)),
            int(settings.ROSE_RED[2] * (0.7 + 0.3 * pulse)),
        )
        text = self.font_title.render(settings.MAIN_TITLE, True, color)
        ...
        self.renderer.get_screen().blit(text, rect)
        sub = self.font_subtitle.render(settings.SUB_TITLE, True, settings.WINE_RED)
        self.renderer.get_screen().blit(
            sub, sub.get_rect(center=(settings.WINDOW_WIDTH // 2, settings.WINDOW_HEIGHT // 2 - 30))
        )
        if int(self.title_pulse * 2) % 2 == 0:
            p = self.font_prompt.render("Press SPACE or ENTER to begin", True, settings.SOFT_PINK)
            self.renderer.get_screen().blit(
                p,
                p.get_rect(center=(settings.WINDOW_WIDTH // 2, settings.WINDOW_HEIGHT // 2 + 100)),
            )
        for i in range(3):
            self.particles._draw_heart(
                self.renderer.get_screen(),
                settings.WINDOW_WIDTH // 2 + (i - 1) * 100,
                settings.WINDOW_HEIGHT // 2 + 180,
                8,
                settings.HEART_RED,
                1.0,
            )
