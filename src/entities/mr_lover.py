...
class MrLover(Character):
    ...
    def draw(self, screen: pygame.Surface):
        ...
        pygame.draw.rect(screen, settings.BLACK, (x+pixel+pixel//2, y+2*pixel+pixel//2 + breath_offset, pixel//2, pixel//2))
        pygame.draw.rect(screen, settings.WHITE, (x+pixel+pixel//2+pixel//4, y+2*pixel+pixel//2 + breath_offset, pixel//4, pixel//4))
        pygame.draw.rect(screen, settings.BLACK, (x+3*pixel+pixel//2, y+2*pixel+pixel//2 + breath_offset, pixel//2, pixel//2))
        pygame.draw.rect(screen, settings.WHITE, (x+3*pixel+pixel//2+pixel//4, y+2*pixel+pixel//2 + breath_offset, pixel//4, pixel//4))
        smile_points = [
        ...
        pygame.draw.rect(screen, settings.BLACK, (x+pixel, y+9*pixel + breath_offset, pixel*2, pixel*3))
        pygame.draw.rect(screen, settings.BLACK, (x+3*pixel, y+9*pixel + breath_offset, pixel*2, pixel*3))
        ...
        if self.holding_rose:
            ...
            pygame.draw.rect(screen, settings.DEEP_RED, (rose_x+pixel//2, rose_y-pixel//2, pixel, pixel))
