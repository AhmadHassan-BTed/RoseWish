...
class MissLovely(Character):
    ...
    def draw_standing(self, screen: pygame.Surface, x: int, y: int, pixel: int):
        ...
        if not blink:
            pygame.draw.rect(screen, settings.BLACK, (x+pixel+pixel//3, y+2*pixel+pixel//2 + breath_offset, pixel//2, pixel//2))
            pygame.draw.rect(screen, settings.WHITE, (x+pixel+pixel//2, y+2*pixel+pixel//2 + breath_offset, pixel//4, pixel//4))
            pygame.draw.rect(screen, settings.BLACK, (x+3*pixel+pixel//3, y+2*pixel+pixel//2 + breath_offset, pixel//2, pixel//2))
            pygame.draw.rect(screen, settings.WHITE, (x+3*pixel+pixel//2, y+2*pixel+pixel//2 + breath_offset, pixel//4, pixel//4))
        else:
            pygame.draw.line(screen, settings.BLACK, 
        ...
        pygame.draw.rect(screen, settings.SOFT_PINK, (x+pixel//2, y+3*pixel + breath_offset, pixel, pixel//2))
        pygame.draw.rect(screen, settings.SOFT_PINK, (x+4*pixel+pixel//2, y+3*pixel + breath_offset, pixel, pixel//2))
        smile_points = [
        ...
        for pos in dress_positions:
            pygame.draw.rect(screen, settings.ROSE_RED, (pos[0], pos[1] + breath_offset, pixel, pixel))
    ...
    def draw_sleeping(self, screen: pygame.Surface, x: int, y: int, pixel: int):
        ...
        pygame.draw.rect(screen, settings.ROSE_RED, (x + 4*pixel, ground_y + 1*pixel, 5*pixel, 3*pixel))
        pygame.draw.rect(screen, settings.ROSE_RED, (x + 4*pixel, ground_y + 1*pixel, pixel, pixel))
